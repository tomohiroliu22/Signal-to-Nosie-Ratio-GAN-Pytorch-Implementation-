...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.noise1 = self.opt.noise1
        self.noise2 = self.opt.noise2
        self.noise3 = self.opt.noise3
        self.noise4 = self.opt.noise4
        print(self.noise1,self.noise2,self.noise3,self.noise4)
    ...
    def forward(self):
        ...
        batchsize, ch_A, h, w = self.real_A.size
        _, ch_B, _, _ = self.real_B.size

         # G_A(A) real A --> fake B
        if(self.noise1 == "YES"):
            noise = torch.rand(batchsize, ch_A, h, w).to(self.device)
            self.noise_real_A = self.real_A + (noise-0.5)/9
            self.noise_real_A[self.noise_real_A > 1] = 1
            self.noise_real_A[self.noise_real_A < -1] = -1
            self.fake_B = self.netG_A(self.noise_real_A) 
        else:
            self.fake_B = self.netG_A(self.real_A)

        # G_B(G_A(A)) fake B --> real A
        if(self.noise2 == "YES"):
            noise = torch.rand(batchsize, ch_B, h, w).to(self.device)
            self.noise_fake_B = self.fake_B + (noise-0.5)/9
            self.noise_fake_B[self.noise_fake_B > 1] = 1
            self.noise_fake_B[self.noise_fake_B < -1] = -1
            self.rec_A = self.netG_B(self.noise_fake_B) 
        else:
        ...
        if(self.noise3 == "YES"):
            ...
            self.noise_real_B[self.noise_real_B < -1] = -1
            self.fake_A = self.netG_B(self.noise_real_B) 
        else:
            self.fake_A = self.netG_B(self.real_B)
        
        # G_A(G_B(B)) fake A --> real B
        if(self.noise4 == "YES"):
            noise = torch.rand(batchsize, ch_A, h, w).to(self.device)
            self.noise_fake_A = self.fake_A + (noise-0.5)/9
            self.noise_fake_A[self.noise_fake_A > 1] = 1
            self.noise_fake_A[self.noise_fake_A < -1] = -1
            self.rec_B = self.netG_A(self.noise_fake_A)   
        else:
